...
# New function for Set MTU
set_mtu() {
    echo "Setting MTU 1420 on all interfaces (except lo)..."
    for iface in $(ls /sys/class/net | grep -v lo); do 
        ip link set dev "$iface" mtu 1420
    done

    echo "Adding @reboot cron job to persist MTU after reboot..."
    (crontab -l 2>/dev/null; echo '@reboot for iface in $(ls /sys/class/net | grep -v lo); do ip link set dev "$iface" mtu 1420; done') | crontab -

    echo "MTU set to 1420 on all interfaces and persistence configured via cron."
}
